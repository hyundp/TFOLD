...
from corl.shared.utils import (compute_mean_std, get_saved_dataset,
                               merge_dictionary, normalize_states)
...
from src.diffusion.utils import (construct_diffusion_model, make_inputs,
                                 split_diffusion_trajectory_woenv)
...
class SimpleDiffusionGenerator:
    ...
    def sample(
        ...
        for i in range(num_batches):
            ...
            if "rewards" in self.modalities:
                obs, actions, rewards, next_obs = split_diffusion_trajectory_woenv(sampled_outputs, obs_dim, action_dim)
                ...
                obs, actions, next_obs =  split_diffusion_trajectory_woenv(sampled_outputs, obs_dim, action_dim)
    ...
    def prepare_sampling_data(
        ...
        return data, cond, terminals, cond_state, cond_reward
    ...
    def sample_back_and_forth(
        ...
        for i in range(num_batches):
            ...
            samples, cond, terminals, cond_state, cond_reward = self.prepare_sampling_data(states,
                                                                              actions,
                                                                              rewards,
                                                                              returns,
                                                                              next_states,
            ...
            if "rewards" in self.modalities:
                gen_state, gen_action, gen_reward, gen_next_obs = split_diffusion_trajectory_woenv(
                ...
                obs, actions, next_obs =  split_diffusion_trajectory_woenv(
                                                        samples = sampled_outputs, 
                                                        obs_dim=self.obs_dim,
                                                        act_dim=self.action_dim,
                                                        modalities = self.modalities,)
    ...
    def sample_back_and_forth_online(
        ...
        if "rewards" in self.modalities:
            gen_obs, gen_action, gen_reward, gen_next_obs = split_diffusion_trajectory_woenv(
                                                                    samples = sampled_outputs, 
                                                                    obs_dim=self.obs_dim,
                                                                    act_dim=self.action_dim,
                                                                    modalities = self.modalities,)
...
if __name__ == '__main__':
    ...
    obs_dim = dataset[0]["observations"].shape[1]
    ...
    fname = "vd4rl/"+args.dataset+"/"+model_nm+"/"+date_+"/"+time_
                    ...
                    if not cfg.SimpleDiffusionGenerator.with_strategy:
                        if cfg.SimpleDiffusionGenerator.weighted_sampling:
                            if "maze" in args.dataset:
                                weighted_sampler=define_rewardweighting_sampler(dataset, args.dataset, cfg.Dataset.reward_scale, cfg.SimpleDiffusionGenerator.weight_param)
                            else:
                                weighted_sampler=define_rewardweighting_sampler(dataset, 
                                                                                args.dataset, 
                                                                                cfg.Dataset.reward_scale, 
                                                                                cfg.SimpleDiffusionGenerator.weight_param,
