...
from src.data.norm import BaseNormalizer, normalizer_factory
from src.diffusion.denoiser_network.mixer import MixerUnet
...
def split_diffusion_trajectory_woenv(
        samples: Union[np.ndarray, torch.Tensor],
        obs_dim: int,
        act_dim: int,
        modalities : List[str] = ["observations", "actions", "rewards"],
...
@gin.configurable
def construct_diffusion_model_gin(
        data_shape: dict,
        inputs,
        normalizer_type,
        denoising_network,
    ...
    #                     d_in=D, 
    #                     cond_dim=cond_dim)
    model = denoising_network(horizon=T,
                              d_in=D, 
                              cond_dim=cond_dim)
    #if disable_terminal_norm:
    #    terminal_dim = D - 1
