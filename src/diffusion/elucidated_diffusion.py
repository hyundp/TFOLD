...
from src.online.utils import make_sequences_from_replay_buffer
...
def discrete_reweighting(episode_rewards,
                         trajectory_returns,
                         trajectory_rewards,
                         reward_scale,
                         episodewise,
    ...
    if episodewise:
        ...
        return episode_rewards, len(episode_rewards)
    
    else:

        trajectory_returns = trajectory_returns.squeeze()
        trajectory_rewards = trajectory_rewards.squeeze()
        assert trajectory_returns.shape == trajectory_rewards.shape
        print(trajectory_returns[trajectory_returns == 0].shape)
        print(trajectory_returns[trajectory_returns > 0].shape)
        trajectory_returns[trajectory_returns == 0] = 1
        trajectory_returns[trajectory_returns > 0] = weight_param
        
        trajectory_returns[trajectory_rewards > 0] *= term_weight
    
        return trajectory_returns, len(trajectory_returns)
...
def define_rewardweighting_sampler(dataset,     
    ...
    trajectory_rewards = dataset.trajectory_rewards

    sparse_reward = ("maze" in dataset_nm)
    
    if sparse_reward:
        weights, wlen = discrete_reweighting(episode_rewards, trajectory_returns, trajectory_rewards, reward_scale, episodewise, weight_param, term_weight)
    else: 
        weights, wlen = ddom_reweighting(trajectory_returns, reward_scale, weight_param, u, q)
        
    sampler = WeightedRandomSampler(torch.DoubleTensor(weights), wlen)
...
def define_terminal_sampler(dataset):                 
    ...
    terminal_idx[terminal_idx == 0] = terminal_rate
    terminal_idx[terminal_idx == 1] = 1 - terminal_rate

    sampler = WeightedRandomSampler(terminal_idx, len(terminal_idx))
...
def apply_state_conditioning(x, cond_state, state_dim):
    for idx, transition in cond_state.items():
        x[idx, -2:, :state_dim] = transition[:,:state_dim].clone()
    return x

def apply_reward_conditioning(x, cond_reward):
    x[:, :, -1] = cond_reward[:,:,-1].clone()
    return x


def apply_sar_conditioning(x, cond_sar):
    for batchidx in cond_sar.keys():
        for timeidx, sar in cond_sar[batchidx].items():
            x[batchidx, timeidx[0]:timeidx[1], :] = sar.clone()
...
class ElucidatedDiffusion(nn.Module):
    ...
    def sample_back_and_forth(
        ...
        if cond_reward is not None:
            assert cond_reward.shape[0] == samples.shape[0]
            cond_reward = cond_reward.to(device=self.device)
            samples = apply_reward_conditioning(samples, cond_reward)
        samples = self.normalizer.normalize(samples)

        norm_reward = samples[:,:,-1].clone().unsqueeze(-1)

        if cond_sar is not None:
            for batchidx in cond_sar.keys():
                for timeidx in cond_sar[batchidx].keys():
                    cond_sar[batchidx][timeidx] = cond_sar[batchidx][timeidx].to(device=self.device)
                    cond_sar[batchidx][timeidx] = self.normalizer.normalize(cond_sar[batchidx][timeidx])
        ...
        for sigma, sigma_next, gamma in tqdm(sigmas_and_gammas[noise_ts:], desc='sampling time step', mininterval=1,
            ...
            # apply condition
            if cond_state is not None:
                inputs_next = apply_state_conditioning(inputs_next, cond_state, state_dim)

            if cond_reward is not None:
                inputs_next = apply_reward_conditioning(inputs_next, norm_reward)
            
            if cond_sar is not None:
                inputs_next = apply_sar_conditioning(inputs_next, cond_sar)
...
class Trainer(object):
    def __init__(
        ...
        self.on_first_state = on_first_state
        self.is_td = is_td
        ...
        with tqdm(initial=self.step, total=self.train_num_steps, disable=not accelerator.is_main_process) as pbar:
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    if self.is_td:
                        states, actions, rewards, next_states, returns, time_steps, terminals, rtg, value = next(self.dl)

                        if self.on_first_state:
                            
                            cond = value[:,0].to(device)
                            cond = cond.reshape(-1, 1)
                        else:
                            cond = value.mean(dim=-1).to(device)
                            cond = cond.reshape(-1, 1)
                        ...
                        if self.discounted_return:
                            if self.on_first_state:
                                ...
                                cond = returns.squeeze().mean(dim=-1).to(device)
                                cond = cond.reshape(-1, 1)
                        else:
...
class REDQTrainer(Trainer):
    ...
    def train_from_redq_buffer(self, buffer , return_scale, guidance_rewardscale, num_steps: Optional[int] = None, seq_len = 31):
        num_steps = num_steps or self.train_num_steps
        for j in range(num_steps):
            b = buffer.sample_sequence_batch(self.batch_size, seq_len=seq_len)
            obs = b['obs1']
            next_obs = b['obs2']
            actions = b['acts']
            rewards = b['rews'][:, :, None]
            ...
            cond = returns[:,0]*return_scale
            cond[cond>0] = cond[cond>0]*guidance_rewardscale
            cond[cond<0] = cond[cond<0]*(-guidance_rewardscale+2)

            # if self.model_terminals:
            #     data.append(done)
            data = [obs, actions, rewards]

            last_state = next_obs[:,-1, None,:]
            last_action = np.zeros_like(actions[:,-1, None,:])
            last_reward = np.zeros_like(rewards[:,-1, None,:])
            last_transition = np.concatenate([last_state, last_action, last_reward], axis=-1)

            data = np.concatenate(data, axis=-1)
            data = np.concatenate([data, last_transition], axis=1)

            # data = np.concatenate(data, axis=-1)
            cond = torch.from_numpy(cond).float()
            data = torch.from_numpy(data).float()
            loss = self.train_on_batch(data, cond=cond, use_wandb=False)
            if j % 1000 == 0:
                print(f'[{j}/{num_steps}] loss: {loss:.4f}')

    def update_normalizer(self, buffer, device=None):

        data = make_sequences_from_replay_buffer(buffer)
        data = torch.from_numpy(data).float()
