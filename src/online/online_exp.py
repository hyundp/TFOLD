...
from REDQ.redq.algos.core import (ReplayTrajectoryBuffer, mbpo_epoches,
                                  test_agent)
from REDQ.redq.utils.bias_utils import log_bias_evaluation
from REDQ.redq.utils.logx import EpochLogger
from REDQ.redq.utils.run_utils import setup_logger_kwargs
from src.diffusion.elucidated_diffusion import REDQTrainer
from src.diffusion.train_diffuser import SimpleDiffusionGenerator
from src.diffusion.utils import construct_diffusion_model_gin
from src.online.redq_rlpd_agent import REDQRLPDAgent
...
def redq_sac(
    ...
    data_shape = {
        "observations": (seq_len+1, obs_dim),
        "actions": (seq_len+1, act_dim),
        "rewards": (seq_len+1, 1),
    }
    print("datashape", data_shape)
    #################################################################################################
    ...
    for t in range(total_steps):
        ...
        if not disable_diffusion and (t + 1) % retrain_diffusion_every == 0 and (t + 1) >= diffusion_start:
            ...
            diffusion_trainer.train_from_redq_buffer(agent.replay_buffer, return_scale = return_scale, guidance_rewardscale = guidance_rewardscale, seq_len=seq_len)
            agent.reset_diffusion_buffer()

            # Add samples to agent replay buffer
            generator = SimpleDiffusionGenerator(env=env, 
                                                 ema_model=diffusion_trainer.ema.ema_model,
                                                 modalities=["observations", "actions", "rewards"],
                                                 )
            
            iterations = int(((num_samples/seq_len)//sample_batch_size)+1)
            print(f"{num_samples}: {sample_batch_size} samples per batch, {iterations} iterations")
            gen_obs_cat, gen_action_cat, gen_reward_cat, gen_next_obs_cat = [], [], [], []
            for i in range(iterations):
                samples = agent.replay_buffer.sample_sequence_batch(batch_size=sample_batch_size,
                                                                    seq_len=seq_len)
                obs1, obs2, acts, rews, done, returns = samples["obs1"], samples["obs2"], samples["acts"], samples["rews"][:, :, None], samples["done"][:, :, None], samples["returns"]

                targets = np.concatenate([obs1, acts, rews], axis=-1)
                last_state = obs2[:, -1, None,:]
                last_action = np.zeros_like(acts[:, -1, None, :])
                last_reward = np.zeros_like(rews[:, -1, None, :])
                last_transition = np.concatenate([last_state, last_action, last_reward], axis=-1)

                targets = np.concatenate([targets, last_transition], axis=1)
                ...
                gen_obs, gen_action, gen_reward, gen_next_obs = generator.sample_back_and_forth_online(
                    targets, cond, obs_dim, num_samples, noise_level, temperature, device
                )
                gen_obs_cat.append(gen_obs)
                gen_action_cat.append(gen_action)
                gen_reward_cat.append(gen_reward)
                gen_next_obs_cat.append(gen_next_obs)
            
            gen_obs = np.concatenate(gen_obs_cat, axis=0)
            gen_action = np.concatenate(gen_action_cat, axis=0)
            gen_reward = np.concatenate(gen_reward_cat, axis=0)
            gen_next_obs = np.concatenate(gen_next_obs_cat, axis=0)


            gen_obs = gen_obs.reshape(-1, obs_dim)
            gen_action = gen_action.reshape(-1, act_dim)
            gen_reward = gen_reward.reshape(-1)
            gen_next_obs = gen_next_obs.reshape(-1, obs_dim)
            

            print(f'Adding {num_samples} samples to replay buffer.')
            agent.diffusion_buffer.store_batch(gen_obs, gen_action, gen_reward, gen_next_obs)
            ...
            if print_buffer_stats:
                ...
                print('Buffer stats:')
                for i in range(gen_obs.shape[1]):
                    print(f'Diffusion Obs {i}: {np.mean(gen_obs[:, i]):.2f} {np.std(gen_obs[:, i]):.2f}')
                    print(
                        f'     Real Obs {i}: {np.mean(real_observations[:, i]):.2f} {np.std(real_observations[:, i]):.2f}')
                for i in range(gen_action.shape[1]):
                    print(f'Diffusion Action {i}: {np.mean(gen_action[:, i]):.2f} {np.std(gen_action[:, i]):.2f}')
                    print(f'     Real Action {i}: {np.mean(real_actions[:, i]):.2f} {np.std(real_actions[:, i]):.2f}')
                print(f'Diffusion Reward: {np.mean(gen_reward):.2f} {np.std(gen_reward):.2f}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='redq_sac_0328')
    ...
    parser.add_argument('--gin_config_files', nargs='*', type=str,
                        default=['configs/online/sac_synther_dmc.gin'])
