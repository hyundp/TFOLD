...
def get_saved_dataset(env: str, dataset ) -> Dict:
        
    with open(f'{DATA_PATH}/original/{env}-{dataset}-v2.pkl','rb') as f:
...
def get_generated_dataset(env: str, step: Optional[int]=None, path: Optional[str]=None) -> Dict:
    if (path is not None) and (path != 'None'):
        ...
        try:
            with open(f'{DATA_PATH}/augmented/{env}.npy','rb') as f:
                data = np.load(f, allow_pickle=True).squeeze() # data는 Dict의 list로 되어 있다.
                file_type = 'npy'
        except:
            with open(f'{DATA_PATH}/augmented/{env}.npz','rb') as f:
    ...
    if 'Dataset' in config_dict.keys():
        ...
        elif config_dict['Dataset']['modalities'].__len__() == 4:
            metadata['generation_type'] = 's,g,a,r'
        else:
            raise ValueError()
        try:
...
def get_FOLD_dataset(env: str, step: Optional[int]=None, path: Optional[str]=None) -> Dict:
    if (path is not None) and (path != 'None'):
        file_type = path.split('.')[-1]
        if file_type == 'npy':    
            data = np.load(path, allow_pickle=True).squeeze()
...
def get_dataset(config):
    ...
    if config.data_mixture_type== 'mixed':
        dataset = get_saved_dataset(config.env, config.dataset)
        ...
        if 'synther' in config.GDA:
            ...
            print('dataset is GTA')
            generated_dataset, metadata = get_GTA_dataset(config.env, config.step, config.datapath)
        elif 'FOLD' in config.GDA:
            print('dataset is transFOLD')
            generated_dataset, metadata = get_FOLD_dataset(config.env, config.step, config.datapath)
        else:
        ...
        print(f"complete (merge original & {config.GDA})")
        return dataset, metadata
    else:
        if config.GDA is None or config.GDA == 'None':
            dataset = get_saved_dataset(config.env, config.dataset)
    ...
    if config.GDA is None or 'None' in config.GDA:
        ...
        if path is not None:
            ...
            if file_type == 'npy':    
                ...
                raise RuntimeError(f"file extension is awkward: {file_type}")
            if config.data_mixture_type=='mixed':
                with open(f'{DATA_PATH}/original/{env}.pkl','rb') as f:
                    ori_dataset = np.load(f, allow_pickle=True)
                    data = (data.tolist() + (ori_dataset))
                    print(f"complete (merge original & {config.GDA})")
        ...
        # print("Dataset size: ", data['observations'].shape[0])
