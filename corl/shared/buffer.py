...
class WeightedReplayBuffer(ReplayBuffer):
    def __init__(
        ...
        super().__init__(
            state_dim, action_dim, buffer_size, device, reward_normalizer, state_normalizer, augmentation
        )
        self._returns = torch.zeros((buffer_size, 1), dtype=torch.float32, device=device)

    def _sample(self, batch_size: int, **kwargs) -> TensorBatch:
        probabilistics = (self._returns[:self._pointer]/self._returns[:self._pointer].sum()).detach().cpu().flatten().numpy()
        indices = np.random.choice(np.arange(self._pointer), size=batch_size, p=probabilistics/probabilistics.sum())
    ...
    def load_d4rl_dataset(self, data: Dict[str, np.ndarray], returns: List[np.ndarray]):
        if not self.empty:
            raise ValueError("Trying to load data into non-empty replay buffer")
        n_transitions = data["observations"].shape[0]
        if n_transitions > self._buffer_size:
        ...
        if 'terminals' in data.keys():
            ...
            self._dones[:n_transitions] = torch.zeros_like(self._rewards[:n_transitions]).to(torch.bool)
        self._pointer = n_transitions
        self._returns[:n_transitions] = self._to_tensor(returns).repeat_interleave(1000).reshape(-1,1) ## ADDED
        self._returns = self._returns - torch.min(self._returns)
...
class DiffusionDataset(Dataset):
    ...
    def _load_d4rl_trajectories(
        ...
        for episode_data in tqdm(dataset) :
            ...
            if self.discounted_return and ("maze" not in self.dataset_nm) and ("pen" not in self.dataset_nm):
                if episode_data["terminals"].any():
                    print("penalty on reward")
                    assert (episode_data["terminals"][-1]) and (not episode_data["terminals"][:-1].any())
                    episode_data["rewards"][-1] -= self.penalty

            if self.discounted_return and ("pen" in self.dataset_nm):
                if episode_data["timeouts"].any():
                    assert (episode_data["timeouts"][-1]) and (not episode_data["timeouts"][:-1].any()) 
                    episode_data["rewards"][-1] -= self.penalty
            ...
            if self.discounted_return and ("maze" not in self.dataset_nm) and ("pen" not in self.dataset_nm)and (self.restore_rewards):
                if episode_data["terminals"].any():
                    assert (episode_data["terminals"][-1]) and (not episode_data["terminals"][:-1].any())
                    episode_data["rewards"][-1] += self.penalty
            
            if self.discounted_return and ("pen" in self.dataset_nm) and (self.restore_rewards):
                if episode_data["timeouts"].any():
                    assert (episode_data["timeouts"][-1]) and (not episode_data["timeouts"][:-1].any()) 
                    episode_data["rewards"][-1] += self.penalty
        ...
        for tr, tlen in zip(traj, traj_len):
            for traj_idx in range(tlen - self.seq_len + 1):
                ...
                if self.discounted_return:
                    if tr["terminals"][start_idx : start_idx + self.seq_len].any() and ("maze" not in self.dataset_nm) and ("pen" not in self.dataset_nm):
                        trajectory_reward = tr["rewards"][start_idx : start_idx + self.seq_len].sum() - 100
                    elif ("timeouts" in tr.keys()) and tr["timeouts"][start_idx : start_idx + self.seq_len].any() and ("pen" in self.dataset_nm):
                        trajectory_reward = tr["rewards"][start_idx : start_idx + self.seq_len].sum() - 100
                    else:
...
class DiffusionTrajectoryDataset(DiffusionDataset):
    def __init__(
        ...
        self.d4rl_info = {
            "terminals" : {
                ...
                "halfcheetah-medium-expert-v2" : 0,
                "halfcheetah-random-v2" : 0,
                "hopper-medium-v2" : 100,
                "hopper-medium-replay-v2" : 100,
                "hopper-medium-expert-v2" : 100,
                "hopper-random-v2" : 100,
                "walker2d-medium-v2" : 100,
                "walker2d-medium-replay-v2" : 100,
                "walker2d-medium-expert-v2" : 100,
                "walker2d-random-v2" : 100,
                "maze2d-umaze-v1" : 0,
                "maze2d-medium-v1" : 0,
                "maze2d-large-v1" : 0,
                "hammer-human-v0": 0,
                ...
                "hammer-human-v1": 0,
                "door-human-v1" : 0,
                "kitchen-partial-v0" : 0,
                "kitchen-mixed-v0" : 0,
                "kitchen-complete-v0" : 0,
                "vd4rl_cheetah_run_expert" : 0,
                "vd4rl_cheetah_run_medium_expert" : 0,
                "vd4rl_cheetah_run_medium" : 0,
                "vd4rl_cheetah_run_medium_replay" : 0,
                "vd4rl_walker_walk_expert" : 0,
                "vd4rl_walker_walk_medium_expert" : 0,
                "vd4rl_walker_walk_medium" : 0,
                "vd4rl_walker_walk_medium_replay" : 0,
                "antmaze-medium-play-v2" : 0,
                "antmaze-medium-diverse-v2" : 0,
                "antmaze-large-play-v2" : 0,
                "antmaze-large-diverse-v2" : 0,
                "antmaze-umaze-v2" : 0,
                "antmaze-umaze-diverse-v2" : 0,
                "vd4rl" : 0
            },
            "timeouts" : {
                "pen-human-v0": 100,
        ...
        if dataset_nm in self.d4rl_info["terminals"].keys():
            self.penalty = self.d4rl_info["terminals"][dataset_nm]
            self.fin_flag = "terminals"
        elif dataset_nm in self.d4rl_info["timeouts"].keys():
            self.penalty = self.d4rl_info["timeouts"][dataset_nm]
            self.fin_flag = "timeouts"
        else:
            print(dataset_nm)
    ...
    def _load_d4rl_trajectories(
        ...
        for episode_data in tqdm(dataset) :
            ...
            if episode_data[self.fin_flag].any():
                terminal_cnt+=1
                non_terminal_cnt-=1

            if self.concat_goal:
                episode_data["observations"] = np.concatenate((episode_data["observations"], episode_data["infos/goal"]), axis=-1)
                episode_data["next_observations"] = np.concatenate((episode_data["next_observations"], episode_data["infos/goal"]), axis=-1)


            elif self.discounted_return:
                if episode_data[self.fin_flag].any():
                    assert (episode_data[self.fin_flag][-1]) and (not episode_data[self.fin_flag][:-1].any()) 
                    episode_data["rewards"][-1] -= self.penalty
            ...
            # for hopper and walker2d
            
            if self.discounted_return and (self.restore_rewards):
                if episode_data[self.fin_flag].any():
                    assert (episode_data[self.fin_flag][-1]) and (not episode_data[self.fin_flag][:-1].any())
                    episode_data["rewards"][-1] += self.penalty
        ...
        for tr, tlen in zip(traj, traj_len):
            for traj_idx in range(tlen - self.seq_len + 1):
                start_idx = traj_idx
                states = tr["observations"][start_idx : start_idx + self.seq_len].astype(np.float32)
                actions = tr["actions"][start_idx : start_idx + self.seq_len].astype(np.float32)
                rewards = tr["rewards"][start_idx : start_idx + self.seq_len].astype(np.float32)
                returns = tr["returns"][start_idx : start_idx + self.seq_len].astype(np.float32)
                terminals = tr[self.fin_flag][start_idx : start_idx + self.seq_len].astype(np.float32)
                time_steps = np.arange(start_idx, start_idx + self.seq_len).astype(np.float32)
                next_states = tr["next_observations"][start_idx : start_idx + self.seq_len].astype(np.float32)
                if "values" in tr.keys():
                    values = tr["values"][start_idx : start_idx + self.seq_len].astype(np.float32)

                rtg = tr["RTG"][start_idx : start_idx + self.seq_len]

                if tr[self.fin_flag][start_idx : start_idx + self.seq_len].any():
                ...
                if "values" in tr.keys():
                    subtrajectories.append([states, actions, rewards, next_states, returns, time_steps, terminals, rtg, values])
                else:
...
class DiffusionTrajectoryFractionDataset(DiffusionDataset):
    def __init__(
            self, 
            dataset: Dict, 
            dataset_nm : str,
        ...
        penalty=100 if penalty is None else penalty
        self.penalty = penalty
        dataset, info = self._load_d4rl_trajectories(dataset, gamma=gamma)
        self.dataset = self._get_fraction_dataset(dataset, info, num_frac = num_frac, frac_id = frac_id)
    ...
    def _get_fraction_dataset(self, dataset, info, num_frac = 10, frac_id = 0):

        trajectory_returns = info["trajectory_returns"].squeeze()
        reward_sort = np.argsort(trajectory_returns)
        frac_gap = len(dataset) // num_frac
        
        idxs = []
        for i in range(num_frac):
            idxs.append((i*frac_gap, (i+1)*frac_gap))
        
        idxs[-1] = (idxs[-1][0], len(dataset))
        
        start_idx = idxs[frac_id][0]
        end_idx = idxs[frac_id][1]
        
        print(f"Fraction Dataset: {start_idx} ~ {end_idx}")
        
        
        return [dataset[idx] for idx in reward_sort[start_idx:end_idx]]
...
def prepare_replay_buffer(
    ...
    if weighted:
        ...
        replay_buffer.load_d4rl_dataset(dataset, returns)
